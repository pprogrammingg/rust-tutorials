...
fn find_maximum_length_from_index(nums: Vec<i32>, curr: usize) -> i32 {
    ...
    if nums[curr] < nums[prev] {
        if (curr + 1) == nums.len() {
            // b.
            println!("b. no more elements. Returning answer as {}", cmp::max(1, nums.len() - 1));
            ...
            println!("c. call find_maximum_length_from_index with  new_nums: {:?} index:{}", new_nums, curr);
            return find_maximum_length_from_index(new_nums, curr);
        }
    } else {
        // d.
        println!("d. call find_maximum_length_from_index with  new_nums: {:?} index:{}", nums, curr + 1);
        if (curr + 1) == nums.len()  {
